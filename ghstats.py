...
def get_pr_stats(repo_owner: str, repo_name: str, since_date: date, until_date: date) -> Tuple[List[Dict[str, any]], List[Dict[str, any]]]:
    """
...
def get_first_commit_date(repo_owner, repo_name, contributor_username) -> date:
    """
...
def get_contributors_stats(env_dict: dict, repo_name: str,
...
def store_contributor_stats(df: pd.DataFrame) -> None:
...
def store_repo_topics(dict_repo_topics: dict) -> None:
    storage_manager: GhsSnowflakeStorageManager = GhsSnowflakeStorageManager()
...
def get_repo_data_over_months() -> None:
    dict_env: dict = get_env()
    if dict_env is None:
    ...
    dict_repo_topics: dict = get_repo_topics(
        dict_env["repo_owner"], repo_names)
    store_repo_topics(dict_repo_topics)
    ...
    for repo in repo_names:
        ...
        df: pd.DataFrame = prepare_for_storage(list_dict_contributors_stats)
        if df is not None:
            store_contributor_stats(df)


if __name__ == "__main__":
    get_repo_data_over_months()
